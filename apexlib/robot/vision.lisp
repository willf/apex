...
;;;
;;; Robot vision
;;; apex/apexlib/robot/vision.lisp
;;;
;;; Copyright:      See apex/LICENSE
;;; Version:        $Id: vision.lisp,v 1.6 2006/01/15 03:42:51 dalal Exp $

;;; Robot vision simulation library.
...
;;; selection, not on how robot vision actually functions.  There is
...
;;; model can't see things that a robot easily could, that will
...
(defmethod initialize-activity ((action seeing) (doer robot))
  (declare (ignorable action))
  (full-visual-update doer))

;;; -- Do a full or partial update  

(defmethod update-activity ((seeing-1 seeing) (robot-1 robot))
  (let* ((last-update (- (current-time) (update-interval seeing-1))))
    ...
    (cond ((or (= (cycle-count seeing-1) (fullcycle seeing-1))
	       (new-gaze-state? (find-resource robot-1 'gaze) last-update)
	       (new-visual-info? (locale robot-1) last-update))
	   (full-visual-update robot-1)
	   (setf (cycle-count seeing-1) 0))
	  (t (partial-visual-update robot-1 last-update)))
    (if (cogevents robot-1) 
	(let ((events  (cogevents robot-1)))
;;;	  (if (> (length events) 1)
;;;	      (print events))
;;;	  (break "check the cogevents of ~a" robot-1)
	  (asamain robot-1)))))
...
;;; RPB 07/19/04 Gaze is not a slot anymore, but a resource.

(defun full-visual-update (robot) 
  (let* ((vfield (vfield (locale robot)))
	 (gaze (find-resource robot 'gaze))
	 (fix (fixation gaze))
	 (attn (locus gaze))
	...
	((process-vattributes (visobs att-types)
	   (loop for visob in visobs append
		 (let ((obfile (get-visobfile visob robot))
		       (alevel (compute-attention-level attn visob))
		       (angle (view-angle (pos visob) fix robot)))
		   (loop for att in att-types
		       when (process-vis-attribute visob obfile att 
				  angle alevel interest intvl robot vfield) 
		       append it)))))
      (let* ((given-atts
...
;;	(print "AT full-vis")
;;	(print all-atts)
	(generate-vision-events all-atts robot)))))
...
(defun partial-visual-update (robot last-update)
  (let* ((locale (locale robot))
	 (gaze (find-resource robot 'gaze))
	 (fix (fixation gaze))
	 (attn (locus gaze))
	 (interest (feature gaze))
...
;;;			 (intersection (cdr entry) att-set :test #'equal)))
			 (list (first entry))))
		      (obfile (get-visobfile visob robot))
		      (alevel (compute-attention-level attn visob))
		      (angle (view-angle (pos visob) fix robot)))
		 (loop for att in att-types
		     when (process-vis-attribute visob obfile att 
						 angle alevel interest intvl robot 
						 (vfield locale))
		     append it)))))
...
;;; pos2 is the fixation/gaze
(defun view-angle (pos1 pos2 agent)
  (if (and pos1 pos2)
      (ang-dist2 (location agent) 
		 (if (= (length pos1) 2)
		     (append pos1 '(0))
		   pos1)
		 (if (= (length pos2) 2)
...
(defun process-vis-attribute (visob obfile att angle attn ftr intvl robot vfield)
  (let* ((oldpstruc (findprop-in-obfile att obfile))
	 (oldvalue 
	  (and 
	   oldpstruc (third (proposition-prop oldpstruc))))
	 (newvalue 
	  (current-vprop-value visob att angle attn ftr intvl robot vfield)))
...
(defun current-vprop-value (visob att angle attn-level ftr intvl robot vfield)
  (let ((trueval 
	 (case att
	   ...
	   (searchmap 
		...
		(compute-searchmap ftr (get-visobfile visob robot)) 
...
;;; RPB 7/19/04 Vision is not a slot anymore

(defun get-visobfile (visob robot)
  (let* ((vmem (memory (find-resource robot 'camera)))
	...
	(let ((newobfile 
	       (make-instance 'visobfile
                              ...
                              :visob visob)))
	  (setf (memory (find-resource robot 'camera))
	    (cons newobfile (memory (find-resource robot 'camera))))
	  newobfile))))
...
;;; Some visobs are associated with unique names.  E.g. {Robot-1} might
...
(defmethod lookup-unique-name (expr (robot robot))
  (let* ((camera (find-resource robot 'camera))
	 (vof (if camera
		  (find-if #'(lambda (v) (equal expr (name (visobfile-visob v))))
		      (memory (find-resource robot 'camera)
;;			      (vision robot)
			      )))))
...
;;  (if  vis-events
;;      (print (list "Vis events " vis-events agent)))
  (let ((anything-new? nil))
    (mapc 
     #'(lambda (event) 
;;	 (cogevent event agent)
	 (cogevent (second event) agent :trigger-asa t)
	 (multiple-value-bind (event2 varies?)
	     (substitute-names event)
	   (if varies? (cogevent event2 agent)))
	 (if (not (equal (first event) 'refreshed))
	     (setf anything-new? t)))
     vis-events)
    (if (not anything-new?) (cogevent '(nothing-new vision) agent))))
...
;;; Related PDL procedures

;;; GAZE

(procedure
 (index (vis-examine ?item))
 (profile (gaze 10 10))
 (step s1 (start-activity gaze fixating :object ?item :duration 250 => ?act))
 ...
 (step s3 (terminate) 
       (waitfor ?hack1
		(nothing-new vision) 
		(held-gaze ?item ?t-held (?if (>= ?t-held 250))))))

;;; more sophisticated variant of above 
(procedure
 (index (vis-examine ?item for at least ?time))
 (profile (gaze 10 10))
 (step s1 (start-activity gaze fixating :object ?item :duration 250 => ?act))
 (step hack1 (waitfor-completion ?act)
       (waitfor ?s1))
 ...
 (step s3 (terminate) 
       (waitfor ?hack1 
		(nothing-new vision) 
		(held-gaze ?item ?t-held (?if (>= ?t-held ?time))))))

;;; ! where did this come from?
;;;(procedure                        
;;; (index (shift-gaze-to ?visob))   
;;; (step s1 (vis-examine ?visob 0))
...
;;; Sets the interest parameter of gaze to specified feature, causing
;;; vision to generate a map (list) of objects in the attended region
;;; with that feature.  The word winnow is from Kosselyn.  Alternative
;;; names that were considered: screen, filter, parse.

(procedure
 (index (winnow ?feature))
 (profile (gaze 10 10))
 (step s1 (start-activity gaze setting-visual-interest :feature ?feature 
			  :duration 250))
 (step s2 (reset +this-task+) (waitfor (resumed +this-task+)))
 (step s3 (terminate >> ?map)
       (waitfor (searchmap ?region ?map))))


;;; Incomplete
;;; RPB: 1/18/04 -- redid the multiple waitfors.
; (procedure
;  (index (read-word ?index ?textblock))
;  (profile (gaze 10 10))
;  (interrupt-cost 6)
;  (step s1 (vis-examine ?textblock 100))
;  (step s2 (signal-resource gaze (feature ?index)) (waitfor ?s1))
;  (step s3 (reset +this-task+) 
;        (waitfor (resumed +this-task+))  ;;reset if continuity broken
;        (waitfor (held gaze ?locus ?time (?if (not (equal ?locus ?textblock))))))
;  (step s4 (terminate >> ?word)
;        (waitfor (text ?textblock ?word))))
...
(procedure
 ...
 (step s3 (reset +this-task+) 
       (waitfor (resumed +this-task+)  ;;reset if continuity broken
		(held gaze ?locus ?time (?if (not (equal ?locus ?textblock))))))
 (step s4 (terminate >> ?word)
       (waitfor (text ?textblock ?word))))
