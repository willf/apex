...
;;;
;;; apex/system/loading/defsystem.lisp
;;;
;;; Copyright:      See apex/LICENSE
;;; Version:        $Id: defsystem.lisp,v 1.2 2006/01/15 03:43:01 dalal Exp $

;;; A portable defsystem implementation gotten off the 'net.
...
(defvar *central-registry*
  `(;; Current directory
    ...
    ;; Home directory
    #-(and :clisp :win32)   ; KMD added this for Apex
...
(defun new-append-directories (absolute-dir relative-dir)
  ...
  (let* ((abs-dir (pathname absolute-dir))
    ...
    ;; KMD: Hack to get Apex loaded in CLISP/Linux.  The logical
    ;; pathname check might be redundant in this case, and a little
    ;; heavy-handed, but I want to localize this patch as much as
    ;; possible, since I don't fully understand the problem.
    ;;    
    #+:CLISP
    (when (and (pathname-logical-p (make-pathname :host host))
               (consp directory)
               (equal "." (car (reverse directory))))
      (setq directory (reverse (cdr (reverse directory))))
      (if (equal rel-file "") (setq rel-file nil)))
